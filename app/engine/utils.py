...
def build_router_system_prompt() -> str:
    ...
    return (
        ...
        "- If user asks for SLIDES from documents -> prefer documents_analyze(..., make_slides=true) or documents_generate_slides(query=...).\n"
        "- If user asks to CREATE/UPDATE slides -> call slides_generate_or_update(prompt=<msg>, max_sections=6).\n"
        "- If user asks to VIEW/OPEN/CHECK slides -> call slides_fetch_latest().\n"
